...
En Fabric, Workspaces → New workspace. Nombre: WS-DP600-Lab.
Dentro del workspace, New item → Lakehouse → nombre LH_Bronze. Deja Lakehouse schemas desactivado.
Repite → LH_Silver.
New item → Warehouse → nombre WH_Gold.
...
New item → Notebook → nombre NB_01_Bronze_Ingesta.
En el panel Explorer izquierdo, Add data items → From OneLake Catalog → selecciona LH_Bronze. Debe quedar como lakehouse por defecto.
Pega y ejecuta la celda siguiente.
...
New item → Notebook → nombre NB_02_Silver_Limpieza.
Añade LH_Silver como lakehouse por defecto en el Explorer.
Ejecuta las celdas siguientes.
...
En el Explorer de WH_Gold, pulsa + Warehouses y añade el SQL analytics endpoint de LH_Silver.
Ejecuta:
-- ============================================================
...
Abre NB_02_Silver_Limpieza y ejecuta esta celda nueva:
...
En WH_Gold, ejecuta el procedimiento y comprueba el resultado:
...
Comprueba ahora el efecto sobre los hechos:
...
New item → pipeline → nombre PL_Medallion.

Añade las actividades en este orden, conectando cada una a la siguiente con la flecha verde (On success):
...
Abre WH_Gold → pulsa en el icono: New semantic model.
Nombre: SM_Ventas. Workspace: WS-DP600-Lab.
Selecciona las tablas: Dim_Date, Dim_Product, Dim_Customer, Dim_Store, Fact_Sales. No incluyas la vista Dim_Order por ahora.
OK → se abre el modelado web.
...
Marcar la tabla de fecha: selecciona Dim_Date → Mark as date table → columna FullDate. Sin esto, las funciones de time intelligence de DAX pueden dar resultados incorrectos.
Ocultar las claves: oculta DateKey, Product_SK, Customer_SK, Store_SK, OrderDateKey, ShipDateKey y todas las columnas Rec*. El usuario no debe verlas.
Ocultar las columnas numéricas del fact (Quantity, NetAmount, …) una vez creadas las medidas, para forzar el uso de las medidas. (haremos esto después de crear las medidas DAX, para no perder de vista los campos mientras las escribes)
Crear la jerarquía de fecha: en Dim_Date, jerarquía Calendario con niveles Year → QuarterName → MonthName → Day.
Ordenar los meses: selecciona MonthName → Sort by column → Month. Si no, los meses se ordenan alfabéticamente.
...
En PL_Medallion, ve a la pestaña Activities → busca "Semantic model refresh" → añádela al lienzo.
Conéctala desde el icono verde ✓ (On success) de 05 - Fact Sales.
Selecciona el bloque nuevo → pestaña Settings.
En Connection, haz clic en el desplegable → Browse all.
En el diálogo "Get data", selecciona la tarjeta "Power BI Semantic Model" (bajo "New sources").
...
Pulsa Connect.
De vuelta en el pipeline, en Settings, verifica que queden rellenos los cuatro campos:
...
En PL_Medallion, ve a la pestaña Activities → busca "Outlook" → añade la actividad Office 365 Outlook (Legacy) al lienzo.

Conéctala arrastrando desde el icono verde ✓ (On success) de Semantic model refresh hasta este nuevo bloque.

Selecciona el bloque → pestaña Settings.

Pulsa Sign in para autenticar la conexión.
...
Con la sesión ya iniciada (verás "Signed in as: tu_correo"), rellena:
...
Los campos de Sensitivity y Advanced (From, Cc, Bcc, Reply to, Importance) se dejan vacíos/por defecto — no son obligatorios.

Guardar y ejecutar

Ve a la cinta Home → botón Save (icono de disquete).
Ve a la pestaña Run → botón Run.
El pipeline se ejecuta de principio a fin (las 7 actividades). Puedes seguir el progreso en el panel inferior Output, con auto-refresh activo.
Al terminar, revisa que las 7 actividades queden en verde (Succeeded), y comprueba tu bandeja de entrada de Outlook para confirmar que llegó el correo de notificación.
image.png
...
Dimensional modeling in Fabric Data Warehouse (overview)
Fact tables
Dimension tables
Load tables
Slowly changing dimension type 2 (Data Factory)
...
Query the warehouse or SQL analytics endpoint (cross-database)
Transactions in Fabric Data Warehouse
Orquestación

How to copy data using Copy activity
Activity overview (Fabric Data Factory)
Use the Semantic model refresh activity
