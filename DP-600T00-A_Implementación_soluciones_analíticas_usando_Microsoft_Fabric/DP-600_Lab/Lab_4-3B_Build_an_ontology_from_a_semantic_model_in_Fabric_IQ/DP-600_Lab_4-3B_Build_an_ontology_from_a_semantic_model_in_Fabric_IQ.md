...
3. In the **New semantic model** dialog, as shown below:
...
3. In the name dialog, enter `LamnaHealthcareOntology`, then select **Create**
...
2. In the **Entity type configuration** pane, under **Key properties**, verify a **HospitalId** key is defined.
3. If no key appears, select **+ Add key**, choose **HospitalId**, and select **Save**.
4. Verify and add keys as needed for the remaining entity types:
...
2. Select the relationship line between **Departments** and **Hospitals** to open the **Relationship configuration** pane.
3. Check the **Source data** section:
...
2. In the **Entity type configuration** pane, go to the **Bindings** tab. You’ll see the existing static binding from the lakehouse.

3. Select **Add data to entity type** to add a second binding.

4. In the **OneLake catalog**, locate **LamnaHealthcareEH** (eventhouse) in your workspace and select it.

5. Select **Add**.

6. Select the **VitalSignsReadings** table and select **Next**.

7. For **Binding type**, select **Time series**.

8. For **Source data timestamp column**, select **Timestamp**.
...
10. Select **Save** to create the time-series binding.
