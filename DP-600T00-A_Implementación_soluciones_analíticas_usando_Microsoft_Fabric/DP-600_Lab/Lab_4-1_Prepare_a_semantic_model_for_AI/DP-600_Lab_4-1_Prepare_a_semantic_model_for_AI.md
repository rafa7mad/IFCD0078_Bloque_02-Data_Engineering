...
Tip: For related training content, see Prepare the semantic layer for AI in Microsoft Fabric.

Set up the environment
Note: You need a paid Fabric capacity to complete this exercise. A Fabric trial doesn’t support Copilot capabilities. For information about Fabric licenses, see Microsoft Fabric licenses.
...
On the Fabric home page, select Power BI.

In the left navigation bar, select Workspaces, and then select New workspace.

Name the new workspace, such as dp_fabric, select a licensing mode that includes Fabric capacity (paid F2 or higher), and select Apply.

Open a web browser and enter the following URL to download the 30-prepare-model-ai zip folder:

https://github.com/MicrosoftLearning/mslearn-fabric/raw/refs/heads/main/Allfiles/Labs/30/30-prepare-model-ai.zip

Save the file in Downloads and extract the zip file to the 30-prepare-model-ai folder.

Open the 30-Starter-Sales Analysis.pbix file from the folder you extracted.

Ignore and close any warnings asking to apply changes. Don’t select Discard changes.

On the Home ribbon, select the Copilot button. When prompted to select a workspace, choose the workspace you created (for example, dp_fabric) and select Connect. This links Power BI Desktop to your Fabric capacity for Copilot features.

On the Home ribbon, locate the Prep data for AI button. If you don’t see it, confirm that you have a current version of Power BI Desktop.
...
On the Home ribbon, select Prep data for AI.

In the dialog that opens, select Simplify the data schema from the left navigation or by selecting the card.

Expand each table and deselect fields that aren’t relevant for business users. By default, all fields are visible to AI. For each table, only keep the following fields:
...
In the Prep for AI dialog, select Settings from the left navigation.

Turn on Share DAX expressions with Copilot. This allows Copilot to read the underlying DAX logic in your measures, improving its ability to choose the correct measure and explain calculations.
...
In the Prep for AI dialog, select Add AI instructions from the left navigation.

In the instructions text box, enter the following business context:
...
Select Apply and then Close to return to the Report view.
...
From the Sales Overview report page, select the card visual that shows Total Sales.

Select the … menu on the visual, then select Set verified answer.

In the verified answer dialog, add the following trigger phrases:
...
Select the bar chart visual that shows Sales by Category and repeat the action to set a verified answer:
...
On the Modeling ribbon, select Q&A setup.

In the Q&A setup dialog, select the Synonyms tab.

From the Customer table, locate CustomerName. Add the following synonyms, pressing Enter after each one:

buyer
client
From the Product table, locate Category. Add the following synonym:
...
From the Sales table, locate the Profit Margin measure. Add the following synonyms:

margin
profit percentage
profitability
Locate the Total Sales measure. Add the following synonyms:

income
revenue
total revenue
Close the Q&A setup dialog.
...
Save the report and select Publish from the Home ribbon.

In the Publish to Power BI dialog, select the workspace you created earlier. Wait for publishing to complete.

When the success message appears, select the link to Open the report in the Power BI service, or navigate to your workspace in a browser.
...
In the Power BI service, open the published report from your workspace.

On the report toolbar, select the Copilot button to open the Copilot pane.

In the Copilot pane, enter your question in Ask a question about this report. If the Understand the data option is available, select it before entering your question.

Ask a question that matches one of your verified answers:

What were total sales?
Copilot should return the predefined card visual showing total sales, rather than generating a new response.

Ask the second verified-answer question:

Show me sales by category
Copilot should return the bar chart visual you created the verified answer for.

Ask a question that tests your synonyms and AI instructions:
...
Select the How Copilot arrived at this answer (HCAAT) link below the response. Review which fields, filters, and measures Copilot used. Confirm that Copilot selected the expected measure and didn’t use any hidden fields.

Ask an open-ended question that doesn’t have a verified answer:
...
If any response is inaccurate, note the question. You can improve accuracy by adjusting AI instructions, adding a verified answer, or refining synonyms.
...
Select the ellipsis (…) for the semantic model item and select Settings.

On the model details page, expand the Approved for Copilot section.

Check the Approved for Copilot option and select Apply.
...
In the workspace settings, select Other and then select Remove this workspace.
